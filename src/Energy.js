...
export default function Energy({ target, hidden = true }) {
  ...
  const graphContainer = createElement({
    type: 'section',
    className: 'graph',
    target: el,
  })
  heading({ text: 'Energy Level throughout the day', target: graphContainer })
  const graphImage = createElement({
    type: 'img',
    className: 'graph__image w-100',
    target: graphContainer,
  })
  graphImage.src = graphImg
  const graphTags = createElement({
    type: 'ul',
    className: 'graph__tag mt-1',
    target: graphContainer,
  })
  graphTags.innerHTML = `
        ...
        `
  const energyContainer = createElement({
    type: 'section',
    className: 'energy',
    target: el,
  })
  heading({ text: 'Rate your current energy level', target: energyContainer })
  const energyRatingSelector = createElement({
    type: 'ul',
    className: 'energy__rating grid-10',
  ...
  function heading({ text, target }) {
    const energyHeading = createElement({
      type: 'h3',
      className: 'graph__heading mb-1',
      target,
    })
